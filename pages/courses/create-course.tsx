...
const CreateCourse: React.FC = () => {
    ...
    const [isAddResourceFormModalOpen, setIsAddResourceFormModalOpen] = useState(false);
    const [isAtualizationClass, setAtualizationClass] = useState(false);
    const [isUpdateClassFormModalOpen, setIsUpdateClassFormModalOpen] = useState(false);
    const [selectedClassResources, setSelectedClassResources] = useState<Resource[]>([]);
    ...
    useEffect(() => {
        ...
        const getResources = async () => {
            if (selectedClass) {
                const resources = await fetchFilteredResources(selectedClass.id as string);
    ...
    const handleCourseSelection = async (selectedCourseId: string, index: number) => {
        ...
        if (selectedClass) {
            ...
            const lastDisplayOrder =
                selectedModuleClasses.length > 0
                    ? Math.max(...selectedModuleClasses.map((c) => c.displayOrder as number))
                    : 0;
            ...
            const selectedModule = selectedCourseModules.find(module => module.UIDModule === selectedModuleId);
    ...
    const handleModuleSubmit = async (values: ModuleFormValues) => {
        if (selectedCourseId) {
            ...
            const moduleId = `${selectedCourseId}-module-${selectedCourseModules.length + 1}`;
    ...
    return (<>
        ...
        <div className="grid grid-cols-4 gap-4 ml-8 mr-8 mt-2">
            ...
            <div>
                ...
                />
                <ActionButton label="Adicionar Curso" onClick={openCourseFormModal} className="mb-4" />
            </div>
            <Modal
            ...
            <div>
                ...
                <ActionButton label="Adicionar Modulo" onClick={openModuleFormModal} className="mb-4" />
                ...
                {selectedModuleClasses &&
                    selectedModuleClasses.map((classItem) => (
                        <div key={classItem.id}>
                            <ClassItem
                                isSelected={selectedClass !== null && classItem.id === selectedClass.id}
                                classItem={classItem}
            ...
            >
                <ClassForm onSubmit={handleClassSubmit} initialValues={selectedClass || undefined} />
            </Modal>
            ...
            <div>
                ...
                {selectedClassResources.map((recurso, index) => (
                    ...
                    // Adicione quaisquer outras funções necessárias para lidar com eventos
                    />
                ))}
                <ActionButton label="Adicionar Recursos" onClick={openAddResourceFormModal} className="mb-4" />
